...
local function getPetValue(tool)
	local ok, result = pcall(function()
		...
		if MutationData[mutation] then
			multiplier = MutationData[mutation].Multiplier or MutationData[mutation].multiplier or MutationData[mutation] or 1
...
local function autoEquipBest()
	...
	-- Unequip all slots and record real coin values from PlacedPart
	for i = 1, slotCount do
		pcall(function()
			local slot = plot.Slots:FindFirstChild("Slot" .. i)
			if slot and slot:FindFirstChild("PlacedPart") then
				table.insert(rankedPets, {
					name  = slot.PlacedPart:GetAttribute("ID"),
					value = parseCoins(slot.PlacedPart:GetAttribute("Coins")),
					fromSlot = true,
				})
	...
	-- Match slot pets to Tool instances, then add remaining backpack pets
	local usedTools = {}
	local finalRanked = {}

	for _, entry in ipairs(rankedPets) do
		for _, tool in ipairs(player.Backpack:GetChildren()) do
			if not usedTools[tool] and tool:IsA("Tool") and tool.Name == entry.name then
				table.insert(finalRanked, { tool = tool, value = entry.value })
				usedTools[tool] = true
				break
			end
		end
	...
	for _, tool in ipairs(player.Backpack:GetChildren()) do
		if tool:IsA("Tool") and not usedTools[tool] then
			table.insert(finalRanked, { tool = tool, value = getPetValue(tool) })
			usedTools[tool] = true
		end
	end

	table.sort(finalRanked, function(a, b) return a.value > b.value end)

	-- Equip best pets into slots
	local filled = 0
	for i = 1, math.min(slotCount, #finalRanked) do
		pcall(function()
			local tool = finalRanked[i].tool
